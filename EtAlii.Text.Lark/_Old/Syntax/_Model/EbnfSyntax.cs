...
/// <summary>
/// Represents parsed syntax from a EBNF (Lark) syntax source.
/// This class contains essential properties and structures
/// such as rules, tokens, imports, and error information resulting from the parsing process.
/// </summary>
public partial class EbnfSyntax
{
    ...
    /// Provides access to all parsed rules extracted from the Lark syntax definitions.
    /// This property contains an array of parsed `Rule` objects, each representing an individual
    /// rule defined in the syntax.
    ...
    /// This property serves as an essential component in analyzing or utilizing the syntax structure.
    public required Rule[] Rules { get; init; }

    /// Contains all tokens parsed from the Lark syntax definitions.
    /// Tokens are individual lexical units identified during the parsing process,
    /// representing named patterns or elements in the syntax syntax.
    ...
    /// This property provides access to all tokens extracted during the syntax analysis.
    ...
    /// reference external syntax files or resources.
    ...
    /// that are to be ignored during the parsing process as dictated by the Lark syntax.
    /// The content of this property is determined during parse-time based on the `!` (ignore) directives
    /// encountered in the input syntax.
    ...
    /// This property contains rules redefined or modified within the syntax,
    /// encapsulated as `OverrideStatement` instances. It provides access to
    /// alterations explicitly applied to existing rules in the parsed syntax.
    ...
    /// This property contains declarations that specify named elements within the syntax definitions.
    ...
    /// and reflects all declarations detected in the input syntax.
    ...
    /// Provides access to the collection of parsed items resulting from the Lark syntax processing.
    ...
    /// generated by processing Lark syntax definitions.
